...
if(document.formularioRegistro.pA.length == 0){
    alert("Por favor escriba su nombre")
    document.formularioRegistro.pA.focus()
    return 0;
}
if(document.formularioRegistro.sA.length == 0){
    alert("Por favor escriba su nombre")
    document.formularioRegistro.sA.focus()
    return 0;
}
...
if (/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3,4})+$/.test(document.formularioRegistro.correo)){
 ...
 var elComboDoctor = document.getElementById("doctor");
